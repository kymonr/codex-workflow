...
Before the first Find spawn, Root freezes one read-only Map brief from the optional Luna scout or from Root's own map: facts, risks, key paths, named refs, and the candidate `as-of` identity. Root injects that same frozen brief into every Find prompt. Finders do not start until the brief exists. Map→Find is a justified barrier because Find needs cross-item Map context. The scout proposes only; Root alone dispatches. The brief is not a claim packet, not a Finder join, and not a license for the scout to spawn. Do not inject the brief into refuter or Sol packets. Independent Finders still must not wait for sibling Finders; item-local refute starts as soon as Root checks that claim.
...
When refute or Sol slots are fewer than surviving material claims, do not invent extra coverage to absorb them. Rank those claims by severity or risk, spend the scarce refute or Sol slots only on the prefix, and log every dropped claim's temporary or stable ID with the reason `verify-cap`. Give that dropped list to the completeness critic. The critic or Root may promote a named dropped claim back onto the verify or Sol path; promotion is not a new search wave and not a repository rescan, and the critic must not hunt new files. `verify-cap` drops whole claims from verify or Sol; it does not replace queuing assigned refuters for a prefix claim when live slots are short of that claim's refute set, and it does not convert a dropped claim into missing-vote `contested`. If no such cap applied, do not invent one.
...
- Starting Finders without injecting the frozen Map brief, or turning Map→Find into a Finder join-all
- Silently dropping claims from a verify cap without logging them or offering critic promotion
- Treating critic promotion of a named dropped claim as a new search wave or a repository rescan
