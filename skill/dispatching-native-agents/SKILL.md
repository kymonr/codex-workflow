...
`wait_agent` has no child-ID target list. When Root is idle, wait once for an agent event, process whichever item updated, check each claim against its citation, and dispatch any triggered item-local adversarial refute immediately while sibling finders continue, unless the Caps whole-claim `verify-cap` wait applies. Sol gates wait until coverage reaches its stop rule and every assigned refuter, including queued refuters, reaches a terminal accounting state; Root then rechecks candidate identity, deduplicates, and assigns stable IDs. Do not delay an independent completed item merely to wait for its siblings.

See [references/patterns.md](references/patterns.md) for the detailed recipe.

### Map brief

When this segment uses a Map-then-Find shape (repository-wide or deep audit, or Root actually produced a scout/map), Root freezes one read-only Map brief from the optional Luna scout or from Root's own map: facts, risks, key paths, named refs, and the candidate `as-of` identity. Root injects that same frozen brief into every Find prompt. Finders in that shape do not start until the brief exists. That Map→Find wait is a justified barrier because those Finders need cross-item Map context. Ordinary fan-out with no Map does not wait for a brief; do not invent a Map child or a brief gate to satisfy this section. The scout proposes only; Root alone dispatches. The brief is not a claim packet, not a Finder join, and not a license for the scout to spawn. Do not inject the brief into refuter or Sol packets. Independent Finders still must not wait for sibling Finders; item-local refute starts as soon as Root checks that claim, unless the Caps whole-claim `verify-cap` wait applies.
...
Live-slot queuing in the previous paragraph is concurrency, not a whole-claim budget. When Root cannot complete a full item-local refute set for every surviving material claim — not counting live-slot queuing that will still start before the pre-dedupe barrier — do not invent extra coverage to absorb them. Decide that whole-claim `verify-cap` before starting item-local refute: wait for coverage to reach its stop rule, then rank those claims by severity or risk. Apply that ranking before assigning refuters to any not-yet-assigned claim; spend refute work only on the prefix, and log every dropped claim's temporary or stable ID with the reason `verify-cap`. Do not start refute in arrival order and later drop later claims. Queuing from the previous Caps paragraph applies only to the selected prefix's refute set. `verify-cap` drops whole extra claims from refute; it does not convert a dropped claim into missing-vote `contested`. After the prefix is selected, each selected claim's item-local refute starts without waiting for its selected siblings. If no such whole-claim cap applies, item-local refute still starts as soon as Root checks that claim.

When Root cannot run a Sol gate for every surviving judgment claim, do not delay item-local refute. Rank that Sol shortage only at the existing post-refute barrier; spend Sol work only on the Sol prefix and log Sol-dropped IDs with reason `verify-cap`. Give the dropped list (refute-dropped and Sol-dropped) to the completeness critic. The critic or Root may promote a named dropped claim back onto the verify or Sol path. A critic `CLAIMS` item that restates a dropped ID, or matches a dropped claim by normalized meaning, is that promotion, not a new claim; do not mint a new temporary anchor for it. Promotion is not a new search wave and not a repository rescan, and the critic must not hunt new files. If no such cap applied, do not invent one.
...
For a Git target, name the audited ref in that snapshot: working-tree `HEAD`, `origin/main`, or another named ref. Record the required read method: working-tree files when the candidate is the checkout, or `git show <ref>:path` when the candidate is a named ref. When the candidate is a named ref, list named untracked exceptions that children may read from the working tree; unlisted untracked files are not that named-ref candidate. When remote or GitHub state is in scope, record a remote as-of: the published SHA, and issue or CI identity when those are examined. Put the audited ref, required read method, named untracked exceptions, and any remote as-of in every child prompt that reads the candidate. Children must not treat a stale checkout as the candidate. Keep repository identity, working-tree `HEAD`, in-scope paths, per-path state, and dirty or untracked fingerprints; published-ref is an added dimension, not a replacement. A change of audited ref, required read method, named untracked exceptions, or in-scope remote as-of is in-scope drift for affected claims. When the candidate is a named ref, a change to working-tree `HEAD` or to fingerprints of files that are not named untracked exceptions is out-of-scope drift for named-ref claims. If the bound audited ref and an in-scope remote as-of SHA differ at freeze, say so in every child prompt that reads the candidate: file bytes follow the required read method on the audited ref; GitHub issue or CI facts follow the remote as-of. Do not mix those sources. That freeze-time split is not later in-scope drift.
...
`completed-empty` applies only when the stop rule is reached and no claim was ever found; spawn no refuters or Sol for that path. If adversarial vote rejects every found claim, list each `C-###` as `REJECT`; that result is not `completed-empty` and defaults to 0 Sol unless a `contested` claim still needs judgment. A completeness critic may ask which class was missed without rescanning. Give it only the coverage packet, named in-scope source inventory, `UNCOVERED`, and any `verify-cap` dropped list. If the critic restates a named dropped ID, or a `CLAIMS` item matches a dropped claim by normalized meaning, treat it as promotion of that ID onto the existing verify or Sol path; do not mint a new temporary anchor or a new claim. Only an evidence-backed claim that is not on the dropped list is a new claim and follows the same adversarial trigger and Sol gate. Material unread evidence withdraws `completed-empty`, makes coverage incomplete, and becomes `UNKNOWN`. Otherwise `completed-empty` stands. The critic does not start another search wave or reset a search cap.
...
For repository-wide or deep-audit refute, a packet pointer that shows one of the following is evidence-backed `unsupported`, not `uncertain`: the cited evidence exists only on a stale working tree and not on the named audited ref; the defect is already absent on the named audited ref; or the claim is only a process smell with no concrete file or command evidence in the packet. Uncertainty remains `uncertain` and does not vote; do not recode it as `unsupported`. These criteria do not replace the three distinct refute questions. When the candidate is a named ref, Root must put in that claim's refute packet bounded payloads for the cited evidence and for `git show` of the bound audited ref at that path (or an explicit absence on that ref). Identity fields in the prompt are not a substitute for those payloads. Refuters still do not rescan. When only two live slots are available on such an audit, those two questions may specialize as existence on the named audited ref versus impact given declared project flags.
...
- Treating a stale checkout as the candidate instead of the named audited ref and its required read method
- Starting Map-then-Find Finders without injecting the frozen Map brief, turning Map→Find into a Finder join-all, or inventing a Map brief gate for ordinary fan-out
- Silently dropping claims from a verify cap without logging them or offering critic promotion
- Treating critic promotion of a named dropped claim as a new search wave, a repository rescan, or a new claim with a new temporary anchor
- Treating the repository-audit Map/Find illustration as a 3+6 quota
- Recoding repo-audit uncertainty as `unsupported`, or replacing the three distinct refute questions with stale-tree / absent-defect / process-smell checks
