...
- 反驳整条预算不够、无法给每条待审结论跑完反驳组时，先决定截断，等覆盖停波后按严重度只反驳前缀，记录 `verify-cap` 丢掉项。现场座位不够仍只排队，不把丢掉的整条记成缺票 `contested`。Sol 名额不足时不推迟反驳，只在反驳结束后的门卫前按严重度截断 Sol。丢掉项交给 completeness critic 点名捞回；捞回是原 ID 重回 verify/Sol，不是新结论、不是新的搜索波，也不准重扫仓库。没有名额压力时不要发明截断。
...
- 仅在 Map-then-Find（仓库深审，或 Root 确实做了 scout/map）时，Scout/Map 产出冻结只读 brief（facts、risks、关键路径、named refs、as-of 身份），Root 注入每个 Find prompt；Finders 等到 brief 存在再开工。普通 fan-out 不等 brief，不要为此发明 Map 子代理。Map→Find 是正当屏障；Scout 不派工；brief 不进 refuter/Sol 包；Find 不等待兄弟 Finders 才开始 item-local 反驳。
