...
Exception: when Root will apply a whole-claim `verify-cap` because it cannot complete a full item-local refute set for every surviving material claim (not counting live-slot queuing that will still start before the pre-dedupe barrier), decide that cap before starting item-local refute, wait for coverage to reach its stop rule, rank, drop the tail with reason `verify-cap`, then start item-local refute only for the prefix. After that prefix is selected, each selected claim's refute starts without waiting for its selected siblings. This join exists solely to spend expensive refute work fairly; it is not a generic Find barrier. Rank a Sol shortage only at the existing post-refute barrier; do not delay item-local refute for Sol capacity. If no such cap applied, do not invent one, and item-local refute still starts as soon as Root checks that claim.
